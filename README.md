...
## Architecture boundary: complement Instinct, never duplicate it

Hermes has exactly four connection classes:

1. **GitHub/workspace** to build approved project files.
2. **OpenAI** to think and generate local code or drafts.
3. **Telegram** to receive Royce's steering and return results or approval requests.
4. **Dedicated Hermes email** to communicate with Instinct at `xt1udv@mail.instinct.com` only.

It must not connect to Royce's Gmail, calendar, contacts, school accounts, Canvas/VHL, Slack, Notion, WhatsApp, or any other personal integration. If Hermes needs current personal context, a deadline, a message, or an account action, it asks Instinct through the email bridge. Startup validation rejects environment variables that indicate one of those prohibited integrations. GitHub access is supplied as a checked-out or mounted repository under `workspace`; Hermes does not need a broad personal-account integration.
